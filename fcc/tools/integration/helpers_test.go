...
//
//	CHAIN_URL       â€” RPC endpoint (default: http://127.0.0.1:8545)
//	ADDRESSES_FILE  â€” path to deployed-addresses.json (default: ../../config/coston2/deployed-addresses.json)
//	DEPLOYMENT_PRIVATE_KEY        â€” funded private key hex (default: Hardhat dev key)
package integration
...
func deployFreshInstructionSender(t *testing.T) (common.Address, *flareseal.FlareSealInstructionSender) {
	t.Helper()
	time.Sleep(rpcDelay)
...
func deployInstructionSenderRaw(t *testing.T, registryAddr, machineRegistryAddr common.Address) error {
	...
	_, _, _, err = flareseal.DeployFlareSealInstructionSender(
		opts, testSupport.ChainClient,

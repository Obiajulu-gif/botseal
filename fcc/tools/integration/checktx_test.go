...
func TestCheckTx_SuccessfulTx(t *testing.T) {
	// Deploy a fresh InstructionSender â€” the deployment tx itself is successful.
	opts, err := bind.NewKeyedTransactorWithChainID(testSupport.Prv, testSupport.ChainID)
	if err != nil {
		t.Fatalf("transactor: %v", err)
	}

	_, tx, _, err := flareseal.DeployFlareSealInstructionSender(
...
func TestCheckTx_FailedTx_DecodesReason(t *testing.T) {
	...
	// Call setExtensionId â€” it will revert because extension is not registered.
	...
	opts.GasLimit = 200000 // Bypass estimation â€” force tx submission

	tx, err := contract.SetExtensionId(opts)
	if err != nil {
		// Some nodes reject at send time even with manual gas â€” can't test
	...
	if receipt.Status == 1 {
		t.Skip("Transaction succeeded (extension might already be registered) â€” cannot test revert path")
	}

	// Now use CheckTx â€” it should detect Status=0 and replay the call to get the reason.
	...
	if strings.Contains(errMsg, "transaction type not supported") {
		t.Skip("Chain does not support replaying this transaction type â€” skipping mined-revert test")
	}
	...
	if strings.Contains(errMsg, "Extension ID not found") {
		...
		// Hex fallback â€” decoded but not as Error(string)
		...
		t.Log("CheckTx detected failure â€” checking if reason is readable")
		// Make sure we don't have binary garbage (non-printable characters)
		for _, r := range errMsg {
			if r < 32 && r != '\n' && r != '\r' && r != '\t' {
				t.Errorf("CheckTx error contains non-printable character (0x%02x) â€” binary garbage not properly decoded", r)
				break
			}
		}
	} else {

/**
 * ★ FlareSeal invoice business logic: validation, integer-cent arithmetic, and the deterministic
 * terms commitment.
 *
 * Everything here runs inside the TEE on the decrypted payload. Nothing in this module may log,
 * echo, or otherwise export the plaintext - errors carry a category, never the offending value.
 *
...
import { encodeAbiParameters, isAddress, getAddress, keccak256, toHex, type Hex } from "viem";
...
/**
 * A validation failure. `message` is deliberately generic: it names the field and the rule, never
 * the submitted value, because it is returned to the caller in `ActionResult.log`.
 */
...
export interface ValidateOptions {
  /** The escrow address this extension is bound to. */
...
export function validateInvoice(
  ...
  if (!isAddress(options.expectedEscrow)) {
    fail("extension is not configured with a valid ESCROW_CONTRACT_ADDRESS");
  }
  if (escrowContract.toLowerCase() !== getAddress(options.expectedEscrow).toLowerCase()) {
    fail("escrowContract does not match this extension's configured escrow");
  }
  ...
  const ceiling = maxTotalCents();
  if (finalTotalCents > ceiling) fail("invoice total exceeds the configured maximum");
